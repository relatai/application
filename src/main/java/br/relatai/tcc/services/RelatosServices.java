...
/**
 * Esta classe de serviço contém as regras de negócio para tratamento de operações 
 * realizadas nos relatos.
 */
@Service
public class RelatosServices {
	...
	// Método que recupera um relato através de um identificador recebido via parâmetro.
	public Relato buscarPorId(String id) {
		// O objeto "relato" é instanciado através do método "findOne()", que recebe o
		// identificador de relato via parâmatro.
	...
	public Relato salvar(Relato relato) {
		// Força o identificador do relato seja nulo para que o banco de dados o atribua.
		relato.setId(null); 
	...
	public ReacaoDTO reagir(String relatoId, Validacao validacao) throws Exception {
		...
		// A classe ReacaoDTO foi criada exclusivamente para dinamizar a exibição		
		// dos somatórios dos atributos de contagem de um relato.
		...
		reacaoDTO.setMensagem("Operação inválida: O sistema não aceita voto do próprio"
				+ " relator ou mais de um voto realizado pelo mesmo usuário.");		
		// Array que não recebe valores duplicados.
	...
	private Validacao salvarValidacao(Validacao validacao) {
		// É necessário recuperarmos o objeto "validacao" com identificador defino no		
		// banco de dados, para que este objeto seja adicionado na lista do objeto relato.
		return validacoesRepository.save(validacao);
	}

	// Método que recebe parâmetros para completar a parte de contabilização das reações. 
	private void operacionalizarReacao(ReacaoDTO reacaoDTO, Relato relato, 
		...
		if (validacao.isReacao()) {
			...
			// O atributo "confirmado" do objeto reacaoDTO recebe o novo valor do atributo 
			// "confirmado" do relato. 
			reacaoDTO.setConfirmado(relato.getConfirmado());
			// Mensagem que será enviada a aplicação cliente.
			...
			// O atributo "denunciado" do objeto reacaoDTO recebe o novo valor do atributo 
			// "denunciado" do relato. 
			reacaoDTO.setDenunciado(relato.getDenunciado());
			// Mensagem que será enviada a aplicação cliente.
			reacaoDTO.setMensagem("Denúncia válida!"); 
			// Etapa que remove o relato caso a quantidade de confirmações seja inferior  
			// a 30 e a quantidade de denúncias seja igual a 5.
			if ((relato.getConfirmado() < 30) && (relato.getDenunciado() == 5)) {
				// O método para remover o relato é invocado.
	...
	public List<Relato> relatosPorUsuario(String usuarioId) {
		...
		// identificador de usuário recebido via parâmetro.
	...
	public void removerSeuProprioRelatoSelecionado(String relatoId) throws Exception {
		// O método "removerRelato()" da regra de negócios de categorias é o método 
		// mais eficiente para remoção de um relato, pois apaga, simultaneamente, o 
		// relato na lista de relatos da collection "categoria" e da collection "relato".
